...
# 3. OPTIONAL: Post-creation Configuration
...
This script will look for a file called `fresh_installation_playbooks.txt` in the root folder. This file should contain a list of Ansible playbooks and their variables that should be run after the servers are created. The script will tell you in advance what playbooks it has found (if any), and what it plans to run.

To skip this step, simply leave blank or delete the `fresh_installation_playbooks.txt` file.

## File format

The format for `fresh_installation_playbooks.txt` is as follows. Each line specifies a playbook name and then the variables you would pass to `--extra-vars`, if any. For example:
...
The hosts keyword 'all' will target all EC2 instances tagged with the project name and environment specified in `terraform.tfvars` (in other words, all the instances created by this script). You can also choose to target individual machines by their name in AWS. For example, if you create two instances and want to configure one to be a webserver and another to be a database, you can target each with `name_<server_name_in_AWS>`:
...
This works thanks to a dynamic inventory file called `ansible/inventory_aws_ec.yaml` that is generated by this script from the template `ansible/inventory_aws_ec.yaml.template`. If you want to change the way that the inventory is filtered or grouped, then modify the template file accordingly. 
